...
define('hikcharts/chart/earth', [ 'require', 'zrender/Group',
			...
			earthGroup.onmousewheel=function(params){
		        ...
		        if(delta>0){
		        	if(currentLevel<maxLevel){
		        		changeNode(cr+(currentLevel-1)*100)
		        		currentLevel++
		        	}
		        	...
		        	if(currentLevel>1){
		        		changeNode(cr+(currentLevel-2)*100)
		        		currentLevel--
		        	}
		        }
			}*/
	...
	var resetZ=function(node,drgee,drgee2){
		...
		var y=node.p.x-cx;
		var z=cy-node.p.y;
		var radius=maxBallRadius;//node.zlevel==4?maxBallRadius+10:maxBallRadius;
		var angel1 = Math.acos(z/radius);
		var angel2 = Math.asin(y/(radius*Math.sin(angel1)));
	...
	var resetRadius=function(node){
		...
		x=Math.round(Math.abs(cr*Math.sin(angel1)*Math.cos(angel2)));
		y=Math.round(cr*Math.sin(angel1)*Math.sin(angel2));
		z=Math.round(cr*Math.cos(angel1));
		node.p.x=cx+y;
		node.p.y=cy-z;
		node.p.z=node.p.z>=0?x:0-x;
		if(isNaN(node.p.z))
	...
	var buildLogo=function(node){
		  ...
		  if(node.zlevel==4){
		    ...
		    circleShape=draw.circle(node.p.x, node.p.y, 2,"#46fffa",opacity,zlevel);
		  circleShape.photo=node.photo;
		  circleShape.nodeId=node.id
		  circleShape.p=node.p;
	...
	var moveLayout=function(drgee,drgee2){
	    ...
	    //console.log("maxBallRadius:"+maxBallRadius);
	    for(var i=0;i<newNodes.length;i++)
		{
			var _node=newNodes[i];
	...
	var displayLayout=function(drgee,drgee2,resetCount){
		if(resetCount){
	...
	var autoChangeLogo=function(node){
		 changeLogo(node,1);
	}
	...
	var bindChangeNode=function(){
	    ...
	    var moveH=function(params) {
			   ...
			   currentX=params.event.clientX;
			   currentY=params.event.clientY;
			   drgee1=(offsetX*180)/(Math.PI*maxBallRadius)
			   drgee2=(offsetY*180)/(Math.PI*maxBallRadius)
			   drgee1=drgee1/100
			   drgee2=drgee2/100
			   var nodes=nodeTable.values();
			   var lines=lineTable.values();
	 ...
	 var loadNode=function(count,drgee,drgee2,isFirstLevel){
		   var limit=1;
		   var iters=isFirstLevel?10:5;
		   var resetCount=isFirstLevel?280:280;
		   var rake=20;
		   var initCount=isFirstLevel?1:10;
		   opt.callback.call(this,"",rake,limit,iters,function(data){
			   ...
			   if(count>0)
				  ...
				  if(drgee+90<360){
					  displayLayout(drgee,0,resetCount);
	...
	var earth = {
			render : function(zrender, option) {
				...
				//if(time)
					//window.clearInterval(time);
				//buildCircle(cx,cy,"#fff",r);
				//buildEarth();
				
				if(opt.callback){
					  var _this=this;
					  loadNode(10,0,0,false);
				}
				//time=window.setInterval(function(){
					//autoChangeNode();
				//}, 500)
