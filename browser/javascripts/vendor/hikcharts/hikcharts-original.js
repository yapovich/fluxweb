			...
			var effect = {
				copyShape : function(shape) {
					...
					if (shape.type == "circle") {
						...
						newShape.style = $.extend({}, shape.style);
						newShape.scale = $.extend({}, shape.scale);
						newShape.position = $.extend({}, shape.position);
						newShape.rotation = $.extend({}, shape.rotation);
					}
					return newShape;
				},
				// ----------------------------------------------------
				...
				line : function(zr, ps, width,color,opacity,zlevel, type) {
					...
					var _lineW = 2
					var effect = {
						scaleSize : require('zrender/tool/env').canvasSupported ? 1
								: 2,
					...
					if (require('zrender/tool/env').canvasSupported) {
						zr.modLayer(2, {
							motionBlur : true,
							lastFrameAlpha : 0.95
						});
					...
					var itemShape = new MarkLineShape(
							{
								style : {
									smooth : (_ps.length <= 2 && type == "spline") ? "spline"
											: "line",
									xStart : _ps[0][0],
									yStart : _ps[0][1], // 坐标
									xEnd : _ps[_ps.length - 1][0],
									yEnd : _ps[_ps.length - 1][1], // 坐标
									brushType : 'both',
									lineType : "solid",
									shadowColor : "#000",// 阴影颜色
									shadowBlur : 0,// 阴影模糊度
									strokeColor : color,// 笔触颜色
									lineWidth : width,// 线条宽度
									symbolSize : [ 0, 1 ],// 端点尺寸
									symbol : [ 'none', 'arrow' ],// 端点样式
									symbolRotate : [ 0, 0 ],
									pointList : _ps.length > 2 ? _ps : null,
									opacity : opacity ? opacity : 1,
								// 端点角度
								},
								zlevel : zlevel?zlevel:0,
								effect : hasEffect ? effect : {},
								smooth : true
							});
					itemShape._echartsData = {}
					zr.addShape(itemShape);
					ecEffect.line(zr, effectList, itemShape, _lineW);
					return itemShape;
				},
				...
				// 辐射圆
				// ----------------------------------------------------
				circle : function(zr, x, y, radius, color) {
					// var hikcharts=require('hikcharts');
					// var zr=hikcharts.zrender;
					var hasEffect = true;
					var interval1
					var interval2;
					var speed = 50;
					...
					var newShape = new CircleShape({
						style : {
							...
							r : 1,
							opacity : opacity,
							color : zrColor.getRadialGradient(x, y, 1, x, y, 1,
									[ [ 0.2, 'rgba(255, 255, 255, 0)' ],
											[ 1, color ] ])
						// zrcolor.getRadialGradient(200,200,1,210,210,10,[[0,"#ff0000"],[1,"#ffffff"]])//'rgba(220,
						// 20, 60, 0.5)'
						}
					});

					if (!hasEffect) {
						newShape.style.r = radius;
						newShape.style.color = zrColor.getRadialGradient(x, y,
								1, x, y, radius, [
						...
						zr.addShape(newShape);
						var secondShape = this.copyShape(newShape);
						zr.addShape(secondShape);
						zr.render();
						var fadeOut = function(shape) {
							var ani = zr.animate(shape.id, "style", false);
							if (ani) {
								ani.when(speed * radius, {
									opacity : 0
								}).done(function() {
						...
						var animate = function(shape) {
							...
							if (shape.style.r >= radius) {
								...
								shape.style.r += 1;
							shape.style.color = zrColor.getRadialGradient(x, y,
									1, x, y, shape.style.r, [
											[ 0.2, 'rgba(255, 255, 255, 0)' ],
											[ 1, color ] ])
...
define('hikcharts/util/draw', ['require'], 
  function(require) {
	...
	var draw = {
		...
		// 线
		// ----------------------------------------------------
		image : function(url, x, y, width, height,opacity,zlevel,handler) {
			var option={
					style : {
						image : url,
						x : x,
					...
					zlevel : zlevel ? zlevel : 0,
					onload:(handler&&handler.onload)?handler.onload:null,
					onclick:(handler&&handler.onclick)?handler.onclick:null,
					onmouseover:(handler&&handler.onmouseover)?handler.onmouseover:null,
					onmouseout:(handler&&handler.onmouseout)?handler.onmouseout:null
				}
			var image = new ImageShape(option);
			return image;
		},
		// ----------------------------------------------------
		// 线
		// ----------------------------------------------------
		line : function(ps, width,color, opacity,zlevel, type) {
			// var zr = this.zrender;
			...
			/*
			 * var shape = new MarkLineShape({ style : { smooth : (_ps.length <=
			 * 2 && type == "spline") ? "spline" : "line", xStart : _ps[0][0],
			 * yStart : _ps[0][1], // 坐标 xEnd : _ps[_ps.length - 1][0], yEnd :
			 * _ps[_ps.length - 1][1], // 坐标 brushType : 'both', lineType :
			 * "solid", strokeColor : color,// 笔触颜色 lineWidth : width,// 线条宽度
			 * symbolSize : [ 0, 1 ],// 端点尺寸 // symbol : [ 'none', 'arrow' ],//
			 * 端点样式 symbol : [ 'none', 'none' ],// 端点样式 symbolRotate : [ 0, 0 ],
			 * pointList : _ps.length > 2 ? _ps : null // 端点角度 }, //
			 * hoverable:false, zlevel : zlevel ? zlevel : 0, smooth : true });
			 */
			// zr.addShape(shape);
			return shape;
		},
		...
		// 圆
		// ----------------------------------------------------
		circle : function(x, y, radius, color, opacity, zlevel) {
			// var zr = this.zrender;
		...
		// 圆环
		// ----------------------------------------------------
		ring : function(x, y, radius1,radius2,color, opacity, zlevel) {
			// var zr = this.zrender;
			var shape = new RingShape({
				style : {
					x : x,
					y : y,
					r0 : radius1,
		...
		// 椭圆
		// ----------------------------------------------------
		ellipse : function(x, y, a, b, color, opacity, zlevel) {
			// var zr = this.zrender;
		...
		// 矩形
		// ----------------------------------------------------
		rectangle : function(x, y, w, h, color, opacity, zlevel, radius) {
			// var zr = this.zrender;
		...
		// 多边形
		// ----------------------------------------------------
		polygon : function(pointList, color, opacity, zlevel) {
			// var zr = this.zrender;
		...
		// 文本
		// ----------------------------------------------------
		text : function(text,font,x, y,color, opacity, zlevel) {
			// var zr = this.zrender;
...
//球体关系图
	define('hikcharts/chart/earth', [ 'require', 'zrender/Group',
		...
		var maxWeight=100;
		var minWeight=0;
		var isAllUnfoldered=false;
		var uint = (2 * Math.PI / 360);//每度代表的弧度
		var nodeHighlightTable=new HashMap();
		var lineHighlightTable=new HashMap();
		var nodeTable=new HashMap();
		var lineTable=new HashMap();
		var nodeShapeTable=new HashMap();
		var lineShapeTable=new HashMap();
		var leafNodes=[];//就绪节点,待展开
		var newNodes=[];//新增节点
		var newLines=[];//新增线段
		var time,time2;
		var earthShape,earthShapeBg;
		var defaultSpeed=500;
		var nodeLoadSpeed=5000;
		var maxRootNodeCount=4;//最大根节点数
		var currentRootNodeCount=0;//当前根节点数
		var maxOtherNodeCount=6;//最大根节点数
		var currentOtherNodeCount=0;//当前根节点数
		var sixBoxCount=12;//六角节点数
		var fourBoxCount=4;//四角节点
		var boxTable=new HashMap();
		var easing = [ "BackIn", "BackInOut", "BackOut", "BounceIn", "BounceInOut",
			"BounceOut", "CircularIn", "CircularInOut", "CircularOut",
			"CubicIn", "CubicInOut", "CubicOut", "ElasticIn", "ElasticInOut",
			"ElasticOut", "ExponentialIn", "ExponentialInOut",
			"ExponentialOut", "Linear", "QuadraticIn", "QuadraticInOut",
			"QuadraticOut", "QuarticIn", "QuarticInOut", "QuarticOut",
		...
		var rainbow =["#05ff00","#0aff00","#0fff00","#14ff00","#19ff00","#1eff00","#23ff00","#28ff00","#2dff00","#33ff00",
			...
			"#9eff00","#a3ff00","#a8ff00","#adff00","#b2ff00","#b7ff00","#bcff00","#c1ff00","#c6ff00","#ccff00",
			"#d1ff00","#d6ff00","#dbff00","#e0ff00","#e5ff00","#eaff00","#efff00","#f4ff00","#f9ff00","#ffff00",
			"#fff900","#fff400","#ffef00","#ffea00","#ffe500","#ffe000","#ffdb00","#ffd600","#ffd100","#ffcc00",
			"#ffc600","#ffc100","#ffbc00","#ffb700","#ffb200","#ffad00","#ffa800","#ffa300","#ff9e00","#ff9900",
			"#ff9300","#ff8e00","#ff8900","#ff8400","#ff7f00","#ff7a00","#ff7500","#ff7000","#ff6b00","#ff6600",
			"#ff6000","#ff5b00","#ff5600","#ff5100","#ff4c00","#ff4700","#ff4200","#ff3d00","#ff3800","#ff3300",
			"#ff2d00","#ff2800","#ff2300","#ff1e00","#ff1900","#ff1400","#ff0f00","#ff0a00","#ff0500","#ff0200"];
		var g_maxBallRadius=0;//全局最大球半径
		var maxBallRadius=0;//最大球半径
		var maxBallX=0;//最大球横向间距
		var maxBallY=0;//最大球纵向间距
		var imageCount=0;
		var imageMaxCount=20;
		var lastMinBallRadiu=300;
		var lastMaxBallRadiu=1000;
		var getGroup=function(){
			if(!earthGroup){
				earthGroup=new Group();
				//earthGroup.scale=[0.1,0.1,cx,cy];
				//var circleShape=draw.circle(cx, cy, r,"#51a81d", 1,-1);
				//earthGroup.clipShape=circleShape;
				zr.addGroup(earthGroup);


				earthGroup.onmousewheel=function(params){
					var zrEvent = require('zrender/tool/event');
					var shape = params.target;
					var event = params.event;
					var delta = zrEvent.getDelta(event);
					if(delta>0){
						if(g_maxBallRadius<lastMaxBallRadiu){
							g_maxBallRadius=(g_maxBallRadius+50)>lastMaxBallRadiu?lastMaxBallRadiu:(g_maxBallRadius+50);
							displayLayout(g_maxBallRadius);
						}
					}
					else{
						if(g_maxBallRadius>lastMinBallRadiu){
							g_maxBallRadius=(g_maxBallRadius-50)<lastMinBallRadiu?lastMinBallRadiu:(g_maxBallRadius-50);
							displayLayout(g_maxBallRadius);
						}
					}
				}

				earthGroup.onmouseover=function(params) {
					clearAutoChangeNode();
				}
				earthGroup.onmouseout=function(params) {
					autoChangeNode();
				}
			}
			return earthGroup;
		...
		var clearGroup=function(){
			zr.clearAnimation();
			clearAutoChangeNode();
			isAllUnfoldered=false;
			boxTable=new HashMap();
			nodeHighlightTable=new HashMap();
			lineHighlightTable=new HashMap();
			nodeTable=new HashMap();
			lineTable=new HashMap();
			nodeShapeTable=new HashMap();
			lineShapeTable=new HashMap();
			leafNodes=[]
			newNodes=[];
			newLines=[];
			earthShapeBg=null;
			earthShape=null;
			g_maxBallRadius=0;//全局最大球半径
			maxBallRadius=0;//最大球半径
			maxBallX=0;//最大球横向间距
			maxBallY=0;//最大球纵向间距
			if(earthGroup){
				earthGroup.clearChildren();
				zr.delGroup(earthGroup);
				earthGroup=null;
			}
		}
		var clearLegendGroup=function(){
			zr.clearAnimation();
			if(legendGroup){
				zr.delGroup(legendGroup);
				legendGroup=null;
			}
		}
		var getDxDy=function(startX,startY,startZ){
			var x,y,z,dx,dy;
			x=startZ;
			y=startX-cx;
			z=cy-startY;
		...
		var getXYByX=function(startX,startY,startZ,drgee,isRad){
			var x0,y0,z0,x,y,z;
			var _drgee=isRad?drgee:uint*drgee;
			x0=startX-cx
			y0=cy-startY;
			z0=startZ;
			x=x0
			y=Math.round(y0*Math.cos(_drgee)-z0*Math.sin(_drgee));
			z=Math.round(y0*Math.sin(_drgee)+z0*Math.cos(_drgee));
			return {x:cx+x,y:cy-y,z:z};
		}
		var getXYByY=function(startX,startY,startZ,drgee,isRad){
			var x0,y0,z0,x,y,z;
			var _drgee=isRad?drgee:uint*drgee;
			x0=startX-cx
			y0=cy-startY;
			z0=startZ;
			x=Math.round(x0*Math.cos(_drgee)+z0*Math.sin(_drgee));
			y=y0;
			z=Math.round(z0*Math.cos(_drgee)-x0*Math.sin(_drgee));
			return {x:cx+x,y:cy-y,z:z};
		}
		var getXY=function(startX,startY){
			var drgeeX=randomBy(100,260)
			var drgeeZ=randomBy(0,180)
			var radius=50
			var x,y,z
			/*if(startX==cx&&startY==cy&&startZ<0){
			 radius=cr;
			 }*/
			x=Math.round(radius*Math.sin(uint*drgeeZ)*Math.sin(uint*drgeeX))
			y=Math.round(radius*Math.cos(uint*drgeeZ))
			z=0;//level<=2?0:Math.round(radius*Math.sin(uint*drgeeZ)*Math.cos(uint*drgeeX))
			var p={x:startX+x,y:startY-y,z:0}
			//var p={x:randomBy(0,zr.getWidth()),y:randomBy(0,zr.getHeight()),z:startZ}
			return p;
			/*
			 var drgee=randomBy(0,360)
			 var lx=lx?lx:50*Math.cos(uint*drgee)//x弧长
			 var ly=ly?ly:50*Math.sin(uint*drgee)//y弧长
			 var _drgeeX=Number((lx*180/(Math.PI*cr)).toFixed(2));
			 var _drgeeY=Number((ly*180/(Math.PI*cr)).toFixed(2));
			 var p=getXYByY(startX,startY,startZ,_drgeeX);
			 p=getXYByX(p.x,p.y,p.z,_drgeeY);
			 var dxy=getDxDy(p.x,p.y,p.z);
			 return {x:p.x,y:p.y,z:p.z,dx:dxy.dx,dy:dxy.dy};*/
			/*
			 var _drgeeX;//=randomBy(-30,30)//绕X轴旋转角度
			 var _drgeeY;//=randomBy(-30,30)//绕Y轴旋转角度
			 if(dx&&dy){
			 //return;
			 _drgeeX=Number((dy*180/(Math.PI*cr)).toFixed(2));
			 _drgeeY=Number((dx*180/(Math.PI*cr)).toFixed(2));
			 }
			 else{

			 var dx=randomBy(10,20)*(randomBy(0,1)==0?1:-1);
			 var dy=randomBy(10,20)*(randomBy(0,1)==0?1:-1);
			 _drgeeX=Number((dx*180/(Math.PI*cr)).toFixed(2));
			 _drgeeY=Number((dy*180/(Math.PI*cr)).toFixed(2));
			 //console.log(dy+","+dx+","+_drgeeX+","+_drgeeY)
			 }
			 var x0,y0,z0,x1,y1,z1,x,y,z;
			 x0=startX-cx
			 y0=cy-startY;
			 z0=startZ;
			 x1=x0
			 y1=Math.round(y0*Math.cos(uint*_drgeeX)-z0*Math.sin(uint*_drgeeX));
			 z1=Math.round(y0*Math.sin(uint*_drgeeX)+z0*Math.cos(uint*_drgeeX));

			 x=Math.round(x1*Math.cos(uint*_drgeeY)+z1*Math.sin(uint*_drgeeY));
			 y=y1;
			 z=Math.round(z1*Math.cos(uint*_drgeeY)-x1*Math.sin(uint*_drgeeY));
			 return {x:cx+x,y:cy-y,z:z};*/
			/*
			 var dx=r*Math.sin(uint*drgeeZ)*Math.cos(uint*drgeeX)//x
			 var dy=r*Math.sin(uint*drgeeZ)*Math.sin(uint*drgeeX)//y
			 var dz=r*Math.cos(uint*drgeeZ)//z
			 return {x:cx+dy,y:cy-dz,z:(drgeeX>=90&&drgeeX<=270)?-2:0};*/
		}
		var getXYR=function(drgee,drgeeX,drgeeZ){
			//3D坐标系与球面坐标系转换
			//x->Y y->Z z->X
			var x0=Math.round(cr*Math.sin(uint*drgeeZ)*Math.sin(uint*drgeeX))
			var y0=Math.round(cr*Math.cos(uint*drgeeZ))
			var z0=Math.round(cr*Math.sin(uint*drgeeZ)*Math.cos(uint*drgeeX))
			console.log("x0:"+x0+",y0:"+y0+",z0:"+z0);
			var a=Math.round(cr*Math.sin(uint*45)*Math.sin(uint*90))
			var b=Math.round(cr*Math.cos(uint*45))
			var c=Math.round(cr*Math.sin(uint*45)*Math.cos(uint*90))
			console.log("a:"+a+",b:"+b+",c:"+c);
			var m00 = Math.cos(drgee*uint) + (a * a) * (1 - Math.cos(drgee*uint));
			var m01 = a * b * (1 - Math.cos(drgee*uint)) - c * Math.sin(drgee*uint);
			var m02 = a * c * (1 - Math.cos(drgee*uint)) + b * Math.sin(drgee*uint);

			var m10 = a * b * (1 - Math.cos(drgee*uint)) + c * Math.sin(drgee*uint);
			var m11 = b * b * (1 - Math.cos(drgee*uint)) + Math.cos(drgee*uint)
			var m12 = b * c * (1 - Math.cos(drgee*uint)) - a * Math.sin(drgee*uint);

			var m20 = a * c * (1 - Math.cos(drgee*uint)) - b * Math.sin(drgee*uint);
			var m21 = b * c * (1 - Math.cos(drgee*uint)) + a * Math.sin(drgee*uint);
			var m22 = c * c * (1 - Math.cos(drgee*uint)) + Math.cos(drgee*uint);

			//console.log((y0*m00+z0*m01+x0*m02)+":"+(y0*m10+z0*m11+x0*m12)+","+(y0*m20+z0*m21+x0*m22));
			/*
			 var x,y,z
			 x=x0*m00+y0*m01+z0*m02//dz
			 y=x0*m10+y0*m11+z0*m12//dx
			 z=x0*m20+y0*m21+z0*m22//dy
			 console.log(drgee+":"+x+","+y+","+z);
			 return {x:cx+y,y:cy-z,z:(drgeeX>=90&&drgeeX<=270)?-2:0};*/
			var x,y,z;
			//绕X轴 360->0 由外向内 0->360 由内向外
			x=x0;
			y=Math.round(y0*Math.cos(uint*drgee)-z0*Math.sin(uint*drgee));
			z=Math.round(y0*Math.sin(uint*drgee)+z0*Math.cos(uint*drgee));
			//绕Y轴 360->0 由右向左 0->360 由左向右
			x=Math.round(x0*Math.cos(uint*drgee)+z0*Math.sin(uint*drgee));
			y=y0;
			z=Math.round(z0*Math.cos(uint*drgee)-x0*Math.sin(uint*drgee));
			//绕Z轴 360->0 顺时针 0->360 逆时针
			//x=Math.round(x0*Math.cos(uint*drgee)-y0*Math.sin(uint*drgee));
			//y=Math.round(x0*Math.sin(uint*drgee)+y0*Math.cos(uint*drgee));
			//z=z0;
			//绕任意轴
			//x=x0*m00+y0*m01+z0*m02
			//y=x0*m10+y0*m11+z0*m12
			//z=x0*m20+y0*m21+z0*m22
			//console.log("x:"+x+",y:"+y+",y:"+y);
			return {x:cx+x,y:cy-y,z:z<0?-3:0};
		}
		var resetZ=function(node,drgee,drgee2){
			var x=0;
			var y=node.p.x-cx;
			var z=cy-node.p.y;
			var radius=node.radius;//node.zlevel==4?maxBallRadius+10:maxBallRadius;
			var angel1 = Math.acos(z/radius);
			var angel2 = Math.asin(y/Math.floor(radius*Math.sin(angel1)));
			x=Math.round(Math.abs(radius*Math.sin(angel1)*Math.cos(angel2)));
			node.p.z=x//node.level<=2?x:0-x;
			if(isNaN(node.p.z)||node.p.z==0){
				node.p.z=0;
				nodeTable.remove(node.id);
				//console.log("删除->"+node.name);
				//node.error=true;
				return;
			}
			if(drgee)
			...
			if(drgee2)
				node.p=getXYByX(node.p.x,node.p.y,node.p.z,drgee2);
		}
		var resetRadius=function(node,radius){
			var x=0;
			var y=node.p.x-cx;
			var z=cy-node.p.y;
			var temp0=z/node.radius
			temp0=temp0>1?1:(temp0<-1?-1:temp0);
			var angel1 = Math.acos(temp0);
			var fm=Math.round(node.radius*Math.sin(angel1))
			var temp=fm==0?0:y/fm;
			temp=temp>1?1:(temp<-1?-1:temp);
			var angel2 = Math.asin(temp);
			if(!isNaN(angel1)&&!isNaN(angel2)){
				x=Math.round(Math.abs(radius*Math.sin(angel1)*Math.cos(angel2)));
				y=Math.round(radius*Math.sin(angel1)*Math.sin(angel2));
				z=Math.round(radius*Math.cos(angel1));
				node.radius=radius;
				node.p.x=cx+y;
				node.p.y=cy-z;
				node.p.z=node.p.z>=0?x:0-x;
				//node.angel1=angel1;
				//node.angel2=angel2;
			}else{
				console.log("异常数据原数据是"+temp+","+node.error);
				node.error=true;
			}
		...
		var regEvent=function(shape){
			var currentX,currentY;
			shape.onmousedown =function(params){
				console.log(params);
				currentX=params.event.x;
				currentY=params.event.y;
				shape.onmousemove=function(params){
					var px=params.event.x-currentX;
					var py=params.event.y-currentY;
					currentX=params.event.x;
					currentY=params.event.y;
					//扇形的弧长=2πr×角度/360
					var drgeeX=Number((px*180/(Math.PI*cr)).toFixed(2));
					var drgeeZ=Number((py*180/(Math.PI*cr)).toFixed(2));
					//console.log(drgeeX+"####"+drgeeZ);
					var nodes=nodeShapeTable.values();
					for(var i=0;i<nodes.length;i++){
						var circleShape=nodes[i];

						if(circleShape.drgeeX+drgeeX>360)
							circleShape.drgeeX=circleShape.drgeeX+drgeeX-360
						else
							circleShape.drgeeX=circleShape.drgeeX+drgeeX
						var r=4;
						var color="#333"
						/*
						 if(((circleShape.drgeeX>=0&&circleShape.drgeeX<=60)||(circleShape.drgeeX>=300&&circleShape.drgeeX<=360))&&(circleShape.drgeeZ>=30&&circleShape.drgeeZ<=150)){
						 r=8
						 color="#51a81d"
						 }*/
						var p=getXY(circleShape.r,circleShape.drgeeZ,circleShape.drgeeX);
						var p0=circleShape.p;
						//circleShape.p=p;
						//console.log("position:"+"["+(p.x-p0.x)+","+(p.y-p0.y)+"]");
						zr.modShape(circleShape.id,{position:[p.x-p0.x,p.y-p0.y],zlevel:p.z,style:{r:r,color:color,opacity:(p.z==-2?0.2:1)}});
						//zr.animate(circleShape.id, "", false).when(500,{position:[p.x-p0.x,p.y-p0.y],zlevel:p.z}).start();
					}
					zr.refresh();
				}
				shape.onmouseup=function(params){
					shape.onmousemove=null;
					shape.onmouseup=null;
				}
				shape.onmouseout=function(params){
					shape.onmousemove=null;
					shape.onmouseup=null;
				}
			}
		}
		var getLineSize=function(strength,isFirstLevel){
			return isFirstLevel?2:1;
			var size=5;
			var range=10;
			var unit=size/range;
			for(var i=0;i<range;i++){
				if(strength>i*range&&strength<=(i+1)*range)
					return unit*(i+1);
			}
		}
		var getColor=function(strength){
			//return "#fff";
			var indx=strength//+randomBy(-30,70);
			indx=indx<=0?0:(indx>=99?99:indx)
			return rainbow[indx];
		}
		var removeNode=function(node){
			var shape=nodeShapeTable.get(node.id);
			nodeShapeTable.remove(node.id)
			var opt;
			if(shape){
				if(shape.type=="circle"){
					opt={
				...
				shape.style.text="";
				zr.animate(shape.id, "style", false).when(defaultSpeed, opt).done(function(){
					shape.clearCache();
					//console.log(shape.type+"清除图片缓存")
					getGroup().removeChild(shape);
					//zr.refresh();
				}).start();
			}
		}
		var removeLine=function(line){
			var shape=lineShapeTable.get(line.startId+"_"+line.endId);
			lineShapeTable.remove(line.startId+"_"+line.endId)
			if(shape){
				zr.animate(shape.id, "style", false).when(defaultSpeed, {
					opacity:0//pointList:[ [ startNode.p.x, startNode.p.y ], [ startNode.p.x+1, startNode.p.y+1 ] ]
				}).done(function(){
					getGroup().removeChild(shape);
					//zr.refresh();
				}).start();
		...
		var removeNodeAndLine=function(){
			var lines=lineHighlightTable.values();
			var nodes=nodeHighlightTable.values();
			console.log("删除所有第一层数据:line's count="+lines.length+",node's count:"+nodes.length);
			lineHighlightTable.clear();
			nodeHighlightTable.clear();
		...
		var buildNode=function(data,startNode,startX,startY,isFirstLevel){
			for(var i=0;i<data.length;i++)
			{
				var line=null;
				var node=null
				if(startNode){
					data[i].strength=data[i].strength+randomBy(-30,70);
					if(data[i].strength<minWeight||data[i].strength>maxWeight)
						continue;
					if(!lineTable.containsKey(data[i].id+"_"+startNode.id)&&!lineTable.containsKey(startNode.id+"_"+data[i].id))
					{
						line=new Object();
						line.drgee=0;
						line.startId=startNode.id;
						line.endId=data[i].id;
						line.size=getLineSize(data[i].strength,isFirstLevel);
						line.weight=data[i].strength;
						line.zlevel=isFirstLevel?3:1
						line.color=!isFirstLevel?"#2d7db8":getColor(data[i].strength);
						newLines.push(line);
						lineTable.put(startNode.id+"_"+data[i].id,line);
					}
				}
				if(nodeTable.containsKey(data[i].id)){
					node=nodeTable.get(data[i].id)
				}else{
					node=new Object();
					node.drgee=0;
					node.id=data[i].id;//"n"+(++initId);
					node.name=data[i].name;
					var p;
					p=getXY(startX,startY);
					node.p=p;
					node.r=16;
					node.startX=startX;
					node.startY=startY;
				...
				if(data[i].children&&data[i].children.length>0){
					buildNode(data[i].children,node,node.p.x,node.p.y,isFirstLevel);
				}

			}
			/*
			 if(startNode){
			 var worker=new Worker("js/vendor/hikcharts/hikcharts_work.js");
			 worker.postMessage({messageName:"data",nodeTable:nodeTable,lineTable:lineTable,nodes:nodeTable.values(),lines:lineTable.values(),width:zr.getWidth(),height:zr.getHeight()});
			 //向worker发送数据
			 worker.onmessage =function(evt){
			 //接收worker传过来的数据函数
			 var nodes=evt.data.nodes;
			 //输出worker发送来的数据
			 //if(evt.data=="reset")
			 nodeTable.clear();
			 for(var i=0;i<nodes.length;i++)
			 nodeTable.put(nodes[i].id,nodes[i]);
			 moveLayout();
		...
		var buildDeepNode=function(){
			console.log("buildDeepNode's length:"+leafNodes.length);
			for(var i=0;i<leafNodes.length;i++){

			}
		}
		var buildEarth=function(radius){
			if(earthShape){
				getGroup().removeChild(earthShapeBg)
				getGroup().removeChild(earthShape)
			}
			earthShapeBg=draw.circle(cx, cy, radius, "#fff",1,0);
			earthShapeBg.hoverable=false;
			earthShapeBg.style.brushType="stroke"
			earthShapeBg.style.strokeColor="#fff"
			earthShapeBg.style.lineWidth=3
			earthShapeBg.style.shadowBlur = 20
			earthShapeBg.style.shadowColor = '#fff'
			earthShapeBg.style.shadowOffsetX = 0
			earthShapeBg.style.shadowOffsetY = 0
			earthShape=draw.circle(cx, cy, radius, "#fff",0,0);
			getGroup().addChild(earthShapeBg)
			getGroup().addChild(earthShape)
			zr.refresh();
		};
		var buildLegend=function(){
			if(!legendGroup){
				legendGroup=new Group();
				zr.addGroup(legendGroup);
				//legendGroup.clearChildren();
				var positionY1=0;
				var positionY2=0;
				var w=15;
				var h=150;
				...
				var minH=h;
				var x=10;
				var y=zr.getHeight()-(h+22);
				var array=[]
				for(var i=1;i<=100;i++){
					array.push([i*0.01,rainbow[100-i]]);
				}
				var color=zrcolor.getLinearGradient(x, y, x, y+h, array);
				var rectangleShape=draw.rectangle(x, y, w,h,color,1,1000);
				var polygonShapeUp=draw.polygon([[x+w,y],[x+w+12,y-12],[x+w+12,y],[x+w,y]],rainbow[99]);
				var polygonShapeDown=draw.polygon([[x+w,y+h],[x+w+12,y+h],[x+w+12,y+h+12],[x+w,y+h]],rainbow[0]);
				var textShapeUp=draw.text("100","12px Arial",x+w+12+5,y-6,"#ffffff");
				var textShapeDown=draw.text("0","12px Arial",x+w+12+5,y+h+6,"#ffffff");
				rectangleShape.hoverable=false;
				polygonShapeUp.hoverable=false;
				polygonShapeUp.draggable=true;
				polygonShapeDown.hoverable=false;
				polygonShapeDown.draggable=true;
				textShapeUp.hoverable=false;
				textShapeDown.hoverable=false;
				legendGroup.addChild(rectangleShape);
				legendGroup.addChild(polygonShapeUp);
				legendGroup.addChild(polygonShapeDown);
				legendGroup.addChild(textShapeUp);
				legendGroup.addChild(textShapeDown);
				legendGroup.width=zr.getWidth();
				legendGroup.height=zr.getHeight();

				polygonShapeUp.ondrift=function(dx,dy){
					positionY1=positionY1+dy;
					positionY1=positionY1<=0?0:(positionY1>minH?minH:positionY1);
					maxH=h-positionY1;
					var p=Math.floor((h-positionY1)*100/h);
					var color=rainbow[p>99?99:p];
					zr.modShape(polygonShapeUp.id, {position : [0, positionY1],style:{color:color}});
					zr.modShape(textShapeUp.id, {position : [0, positionY1],style:{text:p}});
					zr.refresh();
				};
				polygonShapeUp.ondragend=function(){
					positionY1=positionY1<=0?0:(positionY1>minH?minH:positionY1);
					zr.modShape(polygonShapeUp.id, {position : [0, positionY1]});
					zr.modShape(textShapeUp.id, {position : [0, positionY1]});
					zr.refresh();
					maxWeight=Math.floor((h-positionY1)*100/h);
					console.log("minWeight:"+minWeight+",maxWeight:"+maxWeight);
					clearGroup();
					loadNode(5,0,0,false);
				};
				polygonShapeDown.ondrift=function(dx,dy){
					positionY2=positionY2+dy;
					positionY2=positionY2>=0?0:(positionY2+maxH<0?-maxH:positionY2);
					minH=h+positionY2;
					var p=Math.floor(Math.abs(positionY2)*100/h);
					var color=rainbow[p>99?99:p];
					zr.modShape(polygonShapeDown.id, {position : [0, positionY2],style:{color:color}});
					zr.modShape(textShapeDown.id, {position : [0, positionY2],style:{text:p}});
					zr.refresh();
				};
				polygonShapeDown.ondragend=function(){
					positionY2=positionY2>=0?0:(positionY2+maxH<0?-maxH:positionY2);
					zr.modShape(polygonShapeDown.id, {position : [0, positionY2]});
					zr.modShape(textShapeDown.id, {position : [0, positionY2]});
					zr.refresh();
					minWeight=Math.floor(Math.abs(positionY2)*100/h);
					clearGroup();
					loadNode(5,0,0,false);
				};
				zr.refresh();
				...
				var w=zr.getWidth()-legendGroup.width;
				var h=zr.getHeight()-legendGroup.height;
				legendGroup.position=[w,h];
				zr.refresh();
			}
		}
		var buildLogo=function(node){
			var circleShape;
			var opacity=node.p.z>=0?(node.zlevel==4?1:1):0.1;
			var zlevel=node.p.z>=0?node.zlevel:(0-node.zlevel);
			//console.log("opacity:"+opacity+",zlevel:"+zlevel);
			if(node.zlevel==4){
				circleShape = draw.image(node.photo,node.p.x-node.r,node.p.y-node.r,node.r*2,node.r*2,opacity,zlevel,{
					onload:function(){
					}
				});
				/*circleShape = draw.image(node.photo,node.p.x-1,node.p.y-1,1,1,opacity,zlevel,{
				 onload:function(){
				 zr.animate(circleShape.id, "style", false).when(defaultSpeed, {
				 x : node.p.x-node.r,
				 y : node.p.y-node.r,
				 width:node.r*2,
				 height:node.r*2
				...
				circleShape=draw.circle(node.p.x, node.p.y, 2,node.error?"#ff0000":"#46fffa",opacity,zlevel);
			circleShape.photo=node.photo;
			circleShape.nodeId=node.id
			circleShape.p={x:node.p.x,y:node.p.y,z:node.p.z};
			circleShape.style.text=node.name;
			circleShape.style.textColor=node.zlevel==4?"#fff":"rgba(255,255,255,0.2)";
			circleShape.style.textBaseline="middle";
			circleShape.style.shadowBlur = 2
			if(node.zlevel==4){
				circleShape.style.shadowColor = '#000'
				circleShape.style.shadowOffsetX = 1
				circleShape.style.shadowOffsetY = 1
				circleShape.style.textPosition="bottom";
			}else{
				circleShape.style.shadowColor = '#fff'
				circleShape.style.shadowOffsetX = 0
				circleShape.style.shadowOffsetY = 0
				circleShape.style.textPosition="top";
			}
			circleShape.highlightStyle={};
			circleShape.highlightStyle.textColor="#ff0000";
			circleShape.highlightStyle.shadowBlur = 2
			circleShape.highlightStyle.shadowColor = '#000'
			circleShape.highlightStyle.shadowOffsetX = 1
			circleShape.highlightStyle.shadowOffsetY = 1
			getGroup().addChild(circleShape)
			nodeShapeTable.put(node.id,circleShape);
			circleShape.clickable=true;
			if(isClick) {
			...
			/*
			 if(!showImage){
			 zr.animate(circleShape.id, "style", false).when(defaultSpeed, {
			 x : showImage?node.p.x-node.r:node.p.x,
			 y : showImage?node.p.y-node.r:node.p.y
			 }).done(function(){
		...
		{
			var startNode,endNode,key;
			startNode=nodeTable.get(line.startId);
			endNode=nodeTable.get(line.endId);
			key=line.startId+"_"+line.endId;
			if(startNode&&endNode){
				var z=startNode.p.z+endNode.p.z;
				var opacity=z>=0?(line.zlevel==3?1:1):0.1;
				var zlevel=z>=0?line.zlevel:(0-line.zlevel);
				var lineShape = draw.line([ [ startNode.p.x, startNode.p.y ], [ endNode.p.x, endNode.p.y ] ],line.size,line.color,opacity,zlevel);
				if(line.zlevel==3){
					lineShape.style.shadowBlur = 2;
					lineShape.style.shadowColor = "#000";
					lineShape.style.shadowOffsetX = 0;
					lineShape.style.shadowOffsetY = 0;
				}
				getGroup().addChild(lineShape);
				/*
				 zr.animate(lineShape.id, "style", false).when(defaultSpeed, {
				 pointList:[[ startNode.p.x, startNode.p.y ], [ endNode.p.x, endNode.p.y ]]
				 }).done(function(){

				 }).start(easingType);*/
				lineShapeTable.put(key,lineShape);
			}
		};
		var resetLayout=function(){
			var nodes=newNodes;//isback?nodeTable.values():nodeHighlightTable.values();
			var lines=newLines;//isback?lineTable.values():lineHighlightTable.values();
			var area = zr.getWidth() * zr.getHeight();
			var k = Math.sqrt(area/nodes.length);
			var diffx,diffy,diffz,diff;
			var dispx = new HashMap();
			var dispy = new HashMap();
			var ejectfactor = opt.ejectfactor?opt.ejectfactor:6;
			var condensefactor = opt.condensefactor?opt.condensefactor:3;
			var maxtx = 4 ,maxty = 3;
			var visnodeS = null, visnodeE = null;
			var cx = Math.round(zr.getWidth() / 2)
			var cy = Math.round(zr.getHeight() / 2)
			for (var v = 0; v < nodes.length; v++) {
				dispx.put(nodes[v].id, 0);
				dispy.put(nodes[v].id, 0);
				for (var u = 0; u < nodes.length;  u++) {
					if (u != v && nodes[v].p.z==nodes[u].p.z) {
						diffx = nodes[v].p.x - nodes[u].p.x;
						diffy = nodes[v].p.y - nodes[u].p.y;
						diffz = nodes[v].p.z - nodes[u].p.z;
			...
			for (var e = 0; e < lines.length; e++) {
				var eStartID = lines[e].startId;
				var eEndID = lines[e].endId;
				visnodeS = nodeTable.get(eStartID)
				visnodeE = nodeTable.get(eEndID)
				if(visnodeS&&visnodeE){
					diffx = visnodeS.p.x - visnodeE.p.x;
					diffy = visnodeS.p.y - visnodeE.p.y;
					diff = Math.sqrt(diffx * diffx + diffy * diffy);
			...
			for (var v = 0; v < nodes.length; v++) {
				var node = nodes[v];
				var disppx = dispx.get(node.id);
				var disppy = dispy.get(node.id);
				if (disppx < 0-maxtx)
					disppx = 0-maxtx;
				if (disppx > maxtx)
					disppx = maxtx;
				...
				//console.log("disppx:->"+disppx+";disppy:->"+disppy)
				//node.p=getXY(node.p.x,node.p.y,node.p.z,disppx,disppy)
				//node.p.x=node.p.x+disppx;
				//node.p.y=node.p.y+disppy;

				node.p={x:node.p.x+Math.round(disppx),
					y:node.p.y+Math.round(disppy),
					z:node.p.z}
				/*
				 if(node.p.x<30)
				 node.p.x=30;
				 if(node.p.x>zr.getWidth()-100)
				 node.p.x=zr.getWidth()-100;
				 if(node.p.y<40)
				 node.p.y=40;
				 if(node.p.y>zr.getHeight()-50)
				 node.p.y=zr.getHeight()-50;*/
		...
		var moveLayout=function(drgee,drgee2){
			//var count=0;
			var levels=opt.levels?opt.levels:2;
			var speed=opt.speed?opt.speed:defaultSpeed;
			var easingType=opt.easingType?opt.easingType:easingType;
			maxBallX=0;
			maxBallY=0;
			for(var i=0;i<newNodes.length;i++){
				var _node=newNodes[i];
				var offsetX=Math.abs(_node.p.x-cx);
				var offsetY=Math.abs(_node.p.y-cy);
				maxBallX=offsetX>maxBallX?offsetX:maxBallX;
				maxBallY=offsetY>maxBallY?offsetY:maxBallY;
			}
			maxBallRadius=maxBallX>maxBallY?maxBallY:maxBallX;
			//maxBallRadius=Math.round(Math.sqrt(maxBallX * maxBallX + maxBallY * maxBallY));
			//g_maxBallRadius=maxBallRadius>g_maxBallRadius?maxBallRadius:g_maxBallRadius
			for(var i=0;i<newNodes.length;i++)
			{
				var _node=newNodes[i];
				_node.radius=maxBallRadius;
				resetZ(_node,drgee,drgee2)
			}
			/*
			 for(var i=0;i<newNodes.length;i++)
			 {
			 var _node=newNodes[i];
			 var _shape=nodeShapeTable.get(_node.id);
			 //resetXY(_node)
			 //console.log(_node.p);
			 if(_shape){
			 var dx=_node.p.x-_shape.p.x;
			 var dy=_node.p.y-_shape.p.y;
			 //console.log("dx"+dx+"dy"+dy);
			 zr.modShape(_shape.id,{position:[dx,dy]});
			 }else{
			 buildLogo(_node);
			 }
			 }
			 ...
			 {
			 var shape=lineShapeTable.get(newLines[i].startId+"_"+newLines[i].endId);
			 if(shape){
			 var startNode=nodeTable.get(newLines[i].startId);
			 var endNode=nodeTable.get(newLines[i].endId);
			 if(startNode&&endNode)
			 zr.modShape(shape.id,{style:{pointList:[[ startNode.p.x, startNode.p.y ], [ endNode.p.x, endNode.p.y ]]}});
			 }else{
			 buildLine(newLines[i]);
			 }
			 }
			 zr.refresh();*/
			newNodes=[];
		...
		var makeLayout=function(drgee,drgee2,resetCount){
			if(resetCount){
				for(var i=0;i<resetCount;i++)
					resetLayout();
			}
			moveLayout(drgee,drgee2);
		}
		var displayLayout=function(radius){
			buildEarth(radius);
			var newNodes=nodeTable.values();
			var newLines=lineTable.values();
			g_maxBallRadius=radius;
			for(var i=0;i<newNodes.length;i++)
			{
				resetRadius(newNodes[i],radius);
			}
			for(var i=0;i<newNodes.length;i++)
			{
				var _node=newNodes[i];
				...
				if(_shape){
					var dx=_node.p.x-_shape.p.x;
					var dy=_node.p.y-_shape.p.y;
					//console.log("("+_node.p.x+")-("+_shape.p.x+")->dx:"+dx+"dy:"+dy);
					zr.modShape(_shape.id,{position:[dx,dy]});
				}else{
					buildLogo(_node);
				}
			...
			zr.refresh();
			//zr.refresh();
			/*zr.animate(earthGroup.id, "", false).when(defaultSpeed, {
			 scale:[1,1,cx,cy]
			 }).done(function(){
		...
		var clearAutoChangeNode=function(){
			window.clearInterval(time);
			time=null;
		}
		var autoChangeNode=function(){
			if(!isAutoChange)
			   return;
			var d=5/g_maxBallRadius
			var nodes=nodeTable.values();
			var lines=lineTable.values();
			//console.log(d);
			window.clearInterval(time);
			time=window.setInterval(function(){
				for(var i=0;i<nodes.length;i++){
					if(!time){
						console.log("跳出循环，当前节点渲染到："+(i+1)+"/"+nodes.length);
						break;
					}
					autoChangeLogo(nodes[i],d);
				}
				for(var i=0;i<lines.length;i++){
					if(!time){
						//console.log("跳出循环，当前线段渲染到："+(i+1)+"/"+lines.length);
						break;
					}
					autoChangeLine(lines[i]);
				}
				zr.refresh();
			},10);

		}
		var autoChangeLogo=function(node,d){
			changeLogo(node,d,true);
		}
		var autoChangeLine=function(line){
			changeLine(line);
		}
		var changeLogo=function(node,drgee,isRad,drgee2,isRad2){
			var shape=nodeShapeTable.get(node.id);
			if(shape){
				var p=getXYByY(node.p.x,node.p.y,node.p.z,drgee,isRad);//getXY(cr,circleShape.drgeeZ,circleShape.drgeeX);
				if(drgee2)
					p=getXYByX(p.x,p.y,p.z,drgee2,isRad2);
				var p0=shape.p;
				var opacity=p.z>=0?(node.zlevel==4?1:1):0.1;
				var zlevel=p.z>=0?node.zlevel:(0-node.zlevel);
				node.p=p;
				//console.log("drgee:"+node.drgee+"x:->"+p.x+",y:->"+p.y+",z:->"+p.z+",cr:->"+cr+",x-cr:"+(p.x-cx));
				zr.modShape(shape.id,{style:{opacity:opacity},zlevel:zlevel,position:[p.x-p0.x,p.y-p0.y]});
				/*zr.animate(shape.id, "", false).when(10,
				 {position:[p.x-p0.x,p.y-p0.y]})
				 .done(function(){
				 zr.modShape(shape.id,{style:{opacity:opacity},zlevel:zlevel});
				 }).start();*/
			}
		}
		var changeLine=function(line){
			var shape=lineShapeTable.get(line.startId+"_"+line.endId);
			if(shape){
				var startNode=nodeTable.get(line.startId);
				var endNode=nodeTable.get(line.endId);
				var p1=startNode.p//?startNode.dp:getXYByY(startNode.p.x,startNode.p.y,startNode.p.z,drgee,isRad);
				var p2=endNode.p//?endNode.dp:getXYByY(endNode.p.x,endNode.p.y,endNode.p.z,line.drgee,isRad);
				var z=(p1.z<0||p2.z<0)?-1:0;
				var zlevel=z>=0?(line.zlevel):(0-line.zlevel);
				var opacity=z>=0?(line.zlevel==3?1:1):0.1;
				//console.log("line's zlevel:"+zlevel);
				zr.modShape(shape.id,{style:{opacity:opacity,pointList:[[p1.x,p1.y],[p2.x,p2.y]]},zlevel:zlevel});
				/*zr.animate(shape.id, "style", false).when(10,
		...
		var bindChangeNode=function(){
			var config = require('zrender/config');
			var currentX,currentY,offsetX,offsetY;
			var speedPercent=50/300;
			var percent;
			var downH=function(params) {
				//console.log(params.target.type);
				if(params.target&&params.target.type!=="polygon"){
					currentX=params.event.clientX;
					currentY=params.event.clientY;
					percent=Math.round(g_maxBallRadius*speedPercent);
					zr.on(config.EVENT.MOUSEMOVE,moveH);
					zr.on(config.EVENT.MOUSEUP,upH);
				}
			};

			var moveH=function(params) {
				offsetX=params.event.clientX-currentX;
				offsetY=params.event.clientY-currentY;
				currentX=params.event.clientX;
				currentY=params.event.clientY;
				drgee1=(offsetX*180)/(Math.PI*g_maxBallRadius)
				drgee2=(offsetY*180)/(Math.PI*g_maxBallRadius)
				console.log("g_maxBallRadius："+g_maxBallRadius)
				console.log("percent："+percent)
				drgee1=drgee1/percent
				drgee2=drgee2/percent
				var nodes=nodeTable.values();
				var lines=lineTable.values();
				for(var i=0;i<nodes.length;i++){
					changeLogo(nodes[i],drgee1,true,drgee2,true);
				}
				for(var i=0;i<lines.length;i++){
					changeLine(lines[i]);
				}
			...
			var upH=function(params) {
				zr.un(config.EVENT.MOUSEMOVE,moveH);
				zr.un(config.EVENT.MOUSEUP,upH);
			};
			zr.on(config.EVENT.MOUSEDOWN,downH);
		}
		var loadNode=function(count,drgee,drgee2,isFirstLevel){
			var limit=isFirstLevel?1:1;
			var iters=isFirstLevel?10:10;
			var resetCount=isFirstLevel?80:80;
			var rake=isFirstLevel?20:20;
			var initCount=isFirstLevel?1:5;
			opt.callback.call(this,(isFirstLevel&&drgee==0&&opt.rootId)?opt.rootId:"",rake,limit,iters,function(data){
				buildNode(data,null,cx,cy,isFirstLevel);
				count--
				if(count>0)
					loadNode(count,drgee,drgee2,isFirstLevel);
				else{
					if(drgee+90<360){
						makeLayout(drgee,0,resetCount);
						loadNode(initCount,drgee+90,drgee2,isFirstLevel);
					}else{
						if(drgee2==0){
							makeLayout(drgee,0,resetCount);
							loadNode(initCount,drgee,90,isFirstLevel);
						}else if(drgee2==90){
							makeLayout(0,90,resetCount);
							loadNode(initCount,drgee,270,isFirstLevel);
						}else{
							makeLayout(0,270,resetCount);
							if(!isFirstLevel){
								/*if(opt.rootId){
								 opt.callback.call(this,opt.rootId,rake,limit,iters,function(data){
								 if(opt.onLoaded)
								 opt.onLoaded.call(this);
								 buildNode(data,null,cx,cy,true);
								 makeLayout(0,0,resetCount);
								 displayLayout(cr);
								 buildLegend();
								 autoChangeNode();
								 bindChangeNode();
		...
		var earth = {
			render : function(zrender, option) {
				var _self = this;
				zr=zrender;
				opt=option;
				minWeight=0;
				maxWeight=100;
				cx = Math.round(zr.getWidth() / 2);
				cy = Math.round(zr.getHeight() / 2)//-120;
				cr=(cx>cy?cy:cx)-50;
				if(cr<lastMinBallRadiu)
					cr=lastMinBallRadiu;
				isAutoChange=opt.auto;
				isClick=opt.click;
				clearGroup();
				clearLegendGroup();
				if(opt.onLoading)
					opt.onLoading.call(this);
				if(opt.callback){
					var _this=this;
					loadNode(5,0,0,false);
				}
				zr.on("resize",function(){
					buildLegend();
				})
			}
		}
...
define('hikcharts/chart/relation', [ 'require', 'zrender/Group',
	...
	var initId=0;
	var radius=4;
	var radiuses=[20,20,20,20]//[32,24,16,8];
	var opacities=[1,1,1,1];//[1,1,0.5,0.3];
	var opacities2=[1,1,1];//[1,0.5,0.3];
	var defaultSpeed=1000;
	var uint = (2 * Math.PI / 360);//每度代表的弧度
	var lineColor="#000000";
	var colors = ["#f17b49","#3295f2", "#51a81d", "#51a81d", "#51a81d", "#51a81d"]
	var easing = [ "BackIn", "BackInOut", "BackOut", "BounceIn", "BounceInOut",
		   			"BounceOut", "CircularIn", "CircularInOut", "CircularOut",
		   			"CubicIn", "CubicInOut", "CubicOut", "ElasticIn", "ElasticInOut",
		   			"ElasticOut", "ExponentialIn", "ExponentialInOut",
	...
	var rainbow =["#05ff00","#0aff00","#0fff00","#14ff00","#19ff00","#1eff00","#23ff00","#28ff00","#2dff00","#33ff00",
	              ...
	              "#ff6000","#ff5b00","#ff5600","#ff5100","#ff4c00","#ff4700","#ff4200","#ff3d00","#ff3800","#ff3300",
	              "#ff2d00","#ff2800","#ff2300","#ff1e00","#ff1900","#ff1400","#ff0f00","#ff0a00","#ff0500","#ff0200"];
	var getGroup=function(zr){
		if(!relationGroup){
			relationGroup=new Group();
			zr.addGroup(relationGroup);
		}
		return relationGroup;
	}
	var clearRelationGroup=function(zr){
		zr.clearAnimation();
		logoCount=0;
		nodeTable=new HashMap();
		lineTable=new HashMap();
		...
		newLines=[];
		if(relationGroup){
			zr.delGroup(relationGroup);
			relationGroup=null;
		}
	}
	var clearLegendGroup=function(zr){
		zr.clearAnimation();
		if(legendGroup){
			zr.delGroup(legendGroup);
			legendGroup=null;
	...
	var getXY=function(startX,startY,length){
		var _dgree=randomBy(0,360);
		//console.log("_dgree:"+_dgree+";length:"+length);
		var _x;
		var _y;
		if (_dgree == 0) {
			_x = startX + length;
			...
			_y = startY + length;
		} else if (_dgree > 0 && _dgree < 90) {
			_x = startX + length * Math.cos(uint * _dgree)
			_y = startY - length * Math.sin(uint * _dgree)
		} else if (_dgree > 90 && _dgree < 180) {
			_x = startX - length * Math.cos(uint * (180 - _dgree));
			_y = startY - length * Math.sin(uint * (180 - _dgree));
		} else if (_dgree > 180 && _dgree < 270) {
			_x = startX - length * Math.cos(uint * (_dgree - 180));
			_y = startY + length * Math.sin(uint * (_dgree - 180));
		} else if (_dgree > 270 && _dgree < 360) {
			_x = startX + length * Math.cos(uint * (360 - _dgree));
			_y = startY + length * Math.sin(uint * (360 - _dgree));
		}
		return {x:_x,y:_y};
	}
	var getLineSize=function(strength){
		var size=5;
		var range=10;
		var unit=size/range;
		for(var i=0;i<range;i++){
	...
	var getColor=function(strength){
		return rainbow[strength-1];
	}
	var buildNode=function(zr,option,data,startNode,startX,startY){
		initSize=option.initSize?option.initSize:initSize;
		for(var i=0;i<data.length;i++)
		{
		  if(startNode){
			  if(data[i].strength<minWeight||data[i].strength>maxWeight)
			  ...
			  var line=new Object();
	 		  line.startId=startNode.id;
	 		  line.endId=data[i].id;
	 		  line.size=getLineSize(data[i].strength);
	 		  //console.log("line.size:"+line.size);
	 		  line.weight=data[i].strength;
		 	  if(lineTable.containsKey(data[i].id+"_"+startNode.id))
		 	  {
		 		  line=lineTable.get(data[i].id+"_"+startNode.id);
		 		  //line.color="#ff0000";
		 		  //console.log("发现已经画过的线了："+data[i].id+"_"+startNode.id);
		 	  }
		 	  else{
		 		  line.color=getColor(data[i].strength);
		 		  newLines.push(line);
		 		  lineTable.put(startNode.id+"_"+data[i].id,line);
		 		  //console.log("发现已经画过的线了："+data[i].id+"_"+startNode.id);
		      }
		 	  //buildLine(zr,startX,startY,node.x,node.y); 
		  }
		  if(nodeTable.containsKey(data[i].id)){
			  //console.log("发现已经存在的节点了："+data[i].id)
			  if(startNode){
				  if(lineTable.containsKey(startNode.id+"_"+data[i].id)){
					  line=lineTable.get(startNode.id+"_"+data[i].id);
					  //line.color="#00ff00";
				  }	  
			  }
			  continue;
		  }
		  var p=(!startNode&&data.length==1)?{x:startX,y:startY}:getXY(startX,startY,initSize);
		  var node=new Object();
		  node.id=data[i].id;//"n"+(++initId);
		  node.name=data[i].name;
		  ...
		  nodeTable.put(node.id,node);
		  //console.log(node);
	 	  //buildCircle(zr,node.x,node.y);
	 	  //buildLine(zr,startX,startY,node.x,node.y); 
	 	  if(data[i].children&&data[i].children.length){
	 		 buildNode(zr,option,data[i].children,node,node.x,node.y);
	 	  }
	 	  /*
	 	  else if(node.level<4){
	 		 console.log("当前节点的子节点开始读取--->1")
	 		 if(option&&option.callback){
	 			console.log("当前节点的子节点开始读取--->2-"+node.id);
	 			option.callback(node.id,function(data){
	 				console.log("当前节点的子节点开始读取--->3")
	 				console.log(data);
	 				buildNode(zr,option,data,node,node.x,node.y);
	 				//displayLayout(zr,option);
	 			})
	 		 }
	...
	var buildLine=function(zr,key,startX,startY,endX,endY,weight,color,opacity,callback)
	{
		var lineShape = draw.line([ [ startX, startY ], [ endX, endY ] ],weight?weight:1,color,opacity,0);
		//var lineShap=effect.line(zr,[[ startX, startY ], [ endX, endY ]], weight?weight:1,color,opacity,0, "line");
		getGroup(zr).addChild(lineShape);
		/*zr.animate(lineShape.id, "style", false).when(speed, {
	...
	var buildCircle=function(zr,startX,startY,color,radius){
		var circleShape=draw.circle(startX, startY, radius, color?color:"#51a81d", 1,1);
		getGroup(zr).addChild(circleShape)
		//zr.render();
	};
	var buildLogo=function(zr,option,node,startX,startY,width,height,opacity){
		var zlevel=1000-node.level;
		var circleShape=draw.circle(startX, startY, width/2,"#ffffff", opacity,zlevel);
		var ringShape=draw.ring(startX, startY,width/2-1,width/2+1,"#61ae24", opacity,zlevel);
		var imageRingShape = draw.image(edgeBg,startX-(width+10)/2,startY-(height+10)/2,width+10,height+10,opacity,zlevel);
		var imageShape = draw.image(node.photo,startX-width/2,startY-height/2,width,height,opacity,zlevel);
		var imageClipGroup=new Group();
		var imageGroup=new Group();
		imageClipGroup.clipShape=circleShape;
		imageClipGroup.addChild(imageShape);
		imageGroup.addChild(imageClipGroup);
		imageGroup.addChild(ringShape);
		ringShape.style.text=node.name;
		ringShape.style.textColor="#fff";
		ringShape.style.textPosition="bottom";
		ringShape.style.textBaseline="middle";
		imageGroup.addChild(ringShape);
		/*
		getGroup(zr).addChild(imageGroup)
		imageGroup.x=startX;
		imageGroup.y=startY;
		nodeShapeTable.put(node.id,imageGroup);*/
		imageShape.style.text=node.name;
		imageShape.style.textColor="#fff";
		imageShape.style.textPosition="bottom";
		imageShape.style.textBaseline="middle";
		imageShape.x=startX;
		imageShape.y=startY;
		imageShape.draggable=true;
		var speed=option.speed?option.speed:defaultSpeed;
		var easingType=option.easingType?option.easingType:easingType;
		imageShape.ondrift=function(dx,dy){
			node.x=node.x+dx;
			node.y=node.y+dy;
			if(isAllUnfoldered)
			  zr.clearAnimation();
			//zr.animate(imageShape.id, "", false).when(speed,{position:[node.x-imageShape.x,node.y-imageShape.y]}).start(easingType);
		    displayLayout(zr,option,node);
	    };
	    getGroup(zr).addChild(imageShape)
	    nodeShapeTable.put(node.id,imageShape);
	};
	var buildLegend=function(zr,option){
		 if(!legendGroup){
		   legendGroup=new Group();
		   zr.addGroup(legendGroup);
		   //legendGroup.clearChildren();
		   ...
		   var color=zrcolor.getLinearGradient(x, y, x, y+h, array);
		   var rectangleShape=draw.rectangle(x, y, w,h,color,1,1000);
		   var polygonShapeUp=draw.polygon([[x-12,y-12],[x,y],[x-12,y],[x-12,y-12]],rainbow[99]);
		   var polygonShapeDown=draw.polygon([[x-12,y+h],[x,y+h],[x-12,y+h+12],[x-12,y+h]],rainbow[0]);
		   var textShapeUp=draw.text("100","12px Arial",x-35,y-6,"#ffffff");
		   var textShapeDown=draw.text("0","12px Arial",x-35,y+h+6,"#ffffff");
		   rectangleShape.hoverable=false;
		   polygonShapeUp.hoverable=false;
		   polygonShapeUp.draggable=true;
		   polygonShapeDown.hoverable=false;
		   ...
		   polygonShapeUp.ondragend=function(){
		       ...
		       maxWeight=Math.floor((h-positionY1)*100/h);
		       console.log("minWeight:"+minWeight+",maxWeight:"+maxWeight);
		       clearRelationGroup(zr);
		       buildNode(zr,option,option.rootData,null,cx,cy);
			   initLayout(zr,option);
			   displayLayout(zr,option);
		   };
		   polygonShapeDown.ondrift=function(dx,dy){
			    positionY2=positionY2+dy;
	...
	var resetLayout=function(zr,option,node){
		  var nodes=nodeTable.values();
	      var lines=lineTable.values();
		  var area = zr.getWidth() * zr.getHeight();
	      var k = Math.sqrt(area/nodes.length);
	      ...
	      var dispx = new HashMap();
	      var dispy = new HashMap();
	      var ejectfactor = option.ejectfactor?option.ejectfactor:6;
	      var condensefactor = option.condensefactor?option.condensefactor:3;
	      var maxtx = 4 ,maxty = 3;
	      var visnodeS = null, visnodeE = null;
	      var cx = Math.round(zr.getWidth() / 2)
		  var cy = Math.round(zr.getHeight() / 2)
	      for (var v = 0; v < nodes.length; v++) {
	    	    if(node&&nodes[v].id==node.id)
	    	    	continue;
	            dispx.put(nodes[v].id, 0);
	            dispy.put(nodes[v].id, 0);
	            for (var u = 0; u < nodes.length;  u++) {
	       ...
	       for (var v = 0; v < nodes.length; v++) {
	    	    if(node&&nodes[v].id==node.id)
	    	    	continue;
	            var node = nodes[v];
	            var dx = dispx.get(node.id);
	            var dy = dispy.get(node.id);
	            ...
	            if(!disppx){
	              ...
	              //console.log("disppx:"+disppx+",node.x:"+node.x+",node.y:"+node.x);
	              continue;
	            }
	            //console.log("disppx:"+Math.round(disppx)+",disppy:"+Math.round(disppy));
	            
	            node.x=node.x+Math.round(disppx);
	            node.y=node.y+Math.round(disppy);
	            
	            
	            
	            if(node.x<30)
	            	node.x=30;
	            if(node.x>zr.getWidth()-100)
	            	node.x=zr.getWidth()-100;
	            if(node.y<40)
	            	node.y=40;
	            if(node.y>zr.getHeight()-50)
	            	node.y=zr.getHeight()-50;                
	            /*
	            if(node.level>2)
	            {
	              var dx=cx-node.x;
	              var dy=cy-node.y;
	              var r0=(cx>cy?cy:cx)-30
	              var r=Math.sqrt(dx * dx + dy * dy);
	              if(r>r0)
	              {
	            	node.x=cx+dx*r0/r;
	...
	var initLayout=function(zr,option){
		for(var i=0;i<newNodes.length;i++)
		{
			buildLogo(zr,option,newNodes[i],newNodes[i].x,newNodes[i].y,radiuses[newNodes[i].level-1]*2,radiuses[newNodes[i].level-1]*2,opacities[newNodes[i].level-1]);
			//buildCircle(zr,nodes[i].x,nodes[i].y,nodes[i].color,radiuses[nodes[i].level-1]);
		}
		for(var i=0;i<newLines.length;i++)
		{
			var startNode=nodeTable.get(newLines[i].startId);
			...
			buildLine(zr,newLines[i].startId+"_"+newLines[i].endId,startNode.x,startNode.y,endNode.x,endNode.y,newLines[i].size,newLines[i].color,opacities2[startNode.level-1]);
		}
		//console.log("新增连线总数:"+newLines.length+";新增节点总数:"+newNodes.length);
		//console.log("总连线总数:"+lines.length+";总节点总数:"+nodes.length);
		newNodes=[];
		newLines=[];
	}
	var moveLayout=function(zr,option,node){
		var count=0;
		var levels=option.levels?option.levels:2;
		var speed=option.speed?option.speed:defaultSpeed;
		var easingType=option.easingType?option.easingType:easingType;
		var nodes=nodeTable.values();
	    var lines=lineTable.values();
		for(var i=0;i<nodes.length;i++)
		{
			if(node&&nodes[i].id==node.id)
				continue;
			var shape=nodeShapeTable.get(nodes[i].id);
			//console.log(shape)
			var dx=nodes[i].x-shape.x;
			var dy=nodes[i].y-shape.y;
			zr.animate(shape.id, "", false).when(speed,{position:[dx,dy]})
			.done(function(){
				if(!isAllUnfoldered){
				  count++;
				  if(count==nodes.length&&option.callback){
					//var node=nodes[nodes.length-1];
					for(var j=0;j<nodes.length;j++)
				    {
					  if(nodes[j].level>1&&nodes[j].level<=levels&&!nodes[j].isUnfolder){
						//console.log(node);
					    nodes[j].isUnfolder=true;
						option.callback.call(this,nodes[j].id,function(data){
							//console.log(data);
							buildNode(zr,option,data[0].children,nodes[j],nodes[j].x,nodes[j].y);
							initLayout(zr,option);
							displayLayout(zr,option);
						});
						break;
		...
		for(var i=0;i<lines.length;i++)
		{
			var shape=lineShapeTable.get(lines[i].startId+"_"+lines[i].endId);
			if(shape){
			  var startNode=nodeTable.get(lines[i].startId);
			  var endNode=nodeTable.get(lines[i].endId);
			  zr.animate(shape.id, "style", false).when(speed, {
		       pointList:[[ startNode.x, startNode.y ], [ endNode.x, endNode.y ]]
			  })
			  .done(function(){
			  })
			  .start(easingType);
	...
	var displayLayout=function(zr,option,node){
		if(option.resetCount){
			for(var i=0;i<option.resetCount;i++)
			  resetLayout(zr,option,node);
		}
		else{
			for(var i=0;i<300;i++)
			  resetLayout(zr,option,node);
		}
		moveLayout(zr,option,node);
	}
	var relation = {
		render : function(zr, option) {
			var _self = this;
			cx = Math.round(zr.getWidth() / 2)
			cy = Math.round(zr.getHeight() / 2)
			maxWeight=100;
	        minWeight=0;
	        isAllUnfoldered=false;
	        clearRelationGroup(zr);
			clearLegendGroup(zr);
	        buildLegend(zr,option);
			buildNode(zr,option,option.rootData,null,cx,cy);
			initLayout(zr,option);
			displayLayout(zr,option);
			zr.on("resize",function(){
				console.log("zr is resize..............");
				cx = Math.round(zr.getWidth() / 2)
				cy = Math.round(zr.getHeight() / 2)
				buildLegend(zr,option);
			    initLayout(zr,option);
...
define('hikcharts/chart/relation', [ 'require', 'zrender/Group',
	...
	var Group = require('zrender/Group');
	var uint = (2 * Math.PI / 360);//每度代表的弧度
	var radiuses=[24,16,8,4,2,1]
	var colors = ["#f17b49","#3295f2", "#51a81d", "#51a81d", "#51a81d", "#51a81d"]
	var lineColor="#000000";
	var radius=4;
	var opacities=[1,1,1,1,1,1,1];
	...
	var easing = [ "BackIn", "BackInOut", "BackOut", "BounceIn", "BounceInOut",
	   			...
	   			"QuinticIn", "QuinticInOut", "QuinticOut", "SinusoidalIn",
	   			"SinusoidalInOut", "SinusoidalOut" ];
	var easingType = easing[27];// 5,13,14,27,28,29
	...
	var getXY=function(startX,startY,index,zlevel,length){
		var _dgree=randomBy(0,360)//dgrees[index];
		var length=length?length:50;//lengths[index]/(initZlevel-zlevel);
		console.log("_dgree:"+_dgree+";length:"+length);
		var _x;
		var _y;
		if (_dgree == 0) {
	...
	var buildCircles=function(zr,option,data,startX,startY,zlevel,isRoot,length){
		for(var i=0;i<data.length;i++){
			//if(table.containsKey(data[i].id))
			var p=isRoot?{x:startX,y:startY}:getXY(startX,startY,i,zlevel,length);
			if(zlevel<initZlevel)
			  buildLine(zr,startX,startY,p.x,p.y,lineZlevel);
			buildCircle(zr,option,data,p.x,p.y,zlevel);
			if(data[i].children&&data[i].children.length>0)
			  buildCircles(zr,option,data[i].children,p.x,p.y,zlevel-1)
		}
	};
	var buildLine=function(zr,startX,startY,endX,endY,zlevel,callback)
	{
		var lineShape = draw.line([ [ startX, startY ], [ startX-1, startY-1 ] ],1,lineColor,1,zlevel);
	...
	var buildCircle=function(zr,option,data,startX,startY,zlevel){
		var circleShape=draw.circle(startX, startY, radius, "#51a81d", 1, zlevel);
	...
	var relation = {
			render : function(zr, option) {
				...
				var cx = Math.round(zr.getWidth() / 2)
				var cy = Math.round(zr.getHeight() / 2)
				buildCircles(zr,option,option.rootData,cx,cy,initZlevel,option.rootData.length<=1?true:false,150)
			}
	}
	return relation
});*/
...
define('hikcharts/chart/topology', [ 'require', 'zrender/Group',
	...
	var Group = require('zrender/Group');
	var currentScale=1;
	var firstLen = 300;// 第一级线长
	var secondLen = 1 * firstLen;// 第二级线长
	var firstR = 48;// 第一级圆半径
	var secondR = 32;// 第二级圆半径
	var firstBigLen=1000;
	var secondBigLen=2*firstBigLen;
	var thirdR = 16;// 第三级圆半径
	var uint = (2 * Math.PI / 360);//每度代表的弧度
	var speed=1000;
	var colors = [ "#bb0000", "#f23030", "#ff9f19", "#17bf00" ]
	var easing = [ "BackIn", "BackInOut", "BackOut", "BounceIn", "BounceInOut",
			"BounceOut", "CircularIn", "CircularInOut", "CircularOut",
			"CubicIn", "CubicInOut", "CubicOut", "ElasticIn", "ElasticInOut",
			"ElasticOut", "ExponentialIn", "ExponentialInOut",
			...
			"SinusoidalInOut", "SinusoidalOut" ];
	var easingType = easing[27];// 5,13,14,27,28,29
	var radiuses=[83,42,22]
	var lineColor="#3fbde0";
	...
	var buildLine=function(zr,startX,startY,endX,endY,zlevel,callback)
	{
		var lineShape = draw.line([ [ startX, startY ], [ startX-1, startY-1 ] ],zlevel-1,lineColor,opacities[4-zlevel],zlevel);
		zr.addShape(lineShape);
		zr.animate(lineShape.id, "style", false).when(speed, {
			pointList:[[ startX, startY ], [ endX, endY ]]
		}).done(function(){
		...
		//zr.render();
		//effect.line(zr,[ [ startX, startY ], [ endX, endY ] ],3,lineColor,opacities[4-zlevel],zlevel);
	}
	var buildCircle=function(zr,option,data,startX,startY,zlevel){
		var url=zlevel==4?imageUrls[0]:imageUrls[1];
		var r=radiuses[4-zlevel]
		var r2=imageSizes[4-zlevel];
		var imageClipGroup=new Group();
		var imageShape = draw.image(data.url, startX-0.5,startY-0.5,1,1,opacities[4-zlevel], zlevel);
		var circleShape=draw.circle(startX, startY, r2);
		var map=option.map;
		var pmap=option.pmap;
		var imageTextBgShape;
		imageClipGroup.clipShape=circleShape;
		imageClipGroup.addChild(imageShape);
		var imageGroup=new Group();
		var imageBgShape = draw.image(url, startX-0.5,startY-0.5,1,1,opacities[4-zlevel], zlevel,zlevel==4?null:{
			onclick:function(){
				pmap.clear();
				zr.clear();
				zr.clearAnimation();
				...
				cy=cy-65;
				option.rootData=data;
				//buildCircle(zr,option,id,value,cx,cy,4)
				if(option.onclick)
					option.onclick(data.id);

			}
		});
		imageGroup.addChild(imageClipGroup)
		imageGroup.addChild(imageBgShape)
		if(zlevel>=3)
		{
			imageTextBgShape = draw.image(textBgImageUrl, startX,startY+r+14,1,1,opacities[4-zlevel], zlevel,zlevel>=3?{
				onmouseover:function(){
					/*
					imageTextBgShape.style.image=textBgImageUrl2
					imageTextBgShape.style.x=startX-148/2;
					imageTextBgShape.style.y=startY+r;
					imageTextBgShape.style.width=148;
					imageTextBgShape.style.height=58;
					imageTextBgShape.style.text=id;
					zr.render();*/
				},
				onmouseout:function(){
					/*
					imageTextBgShape.style.image=textBgImageUrl
					imageTextBgShape.style.x=startX-78/2;
					imageTextBgShape.style.y=startY+r;
					imageTextBgShape.style.width=78;
					imageTextBgShape.style.height=28;
					imageTextBgShape.style.text=id;
					zr.render();*/
				}
			}:null);
			imageTextBgShape.style.text=(zlevel==4?"*"+data.id.substr(24):data.value);
			imageTextBgShape.style.textColor="#fff";
			imageTextBgShape.style.textPosition="inside";
			imageTextBgShape.style.textBaseline="middle";
			imageTextBgShape.zlevel=10;
			imageGroup.addChild(imageTextBgShape)
		}
		zr.addGroup(imageGroup);
		//var circleShape=draw.circle(startX, startY, radiuses[4-zlevel], colors[zlevel-1], opacities[4-zlevel], zlevel);
		//zr.addShape(circleShape);
		/*zr.render();*/		
		
		if(zlevel>=3)
		{
			zr.animate(imageTextBgShape.id, "style", false).when(speed, {
				x : startX-78/2,
				y : startY+r,
				width : 78,
				height : 28
		...
		//if(zlevel>=3)
		{
		  imageBgShape.rotation=[0,startX,startY];
		  zr.animate(imageBgShape.id, "", true).when(500000, {rotation:[360,startX,startY]}).start();
		}
		zr.animate(imageBgShape.id, "style", false).when(speed, {
			x : startX - r,
			y : startY - r,
			width : r * 2,
			height : r * 2
		}).done(function(){
		 pmap.put(data.id, [ startX,startY ]);
         zlevel--
         if(zlevel<2)
         {
        	return;
         ...
         {
        	var sons = map.get(data.id);
        	if(sons&&sons.length>0)
        	{
        		if(zlevel==3&&option.callback)
        		{
        			option.callback(data,sons);
        		}
        		for(var i=0;i<sons.length;i++)
        		{
        			if(sons[i].id==option.rootData.id)
        				continue;
        			if(!pmap.get(sons[i].id))
        			{
        				var p=getXY(startX,startY,i);
        				var p2=getXY(startX,startY,i,radiuses[3-zlevel]);
        				var p3=getXY(startX,startY,i,lengths[i]-radiuses[4-zlevel]);
            			buildLine(zr,p2.x,p2.y,p3.x,p3.y,zlevel);
            			buildCircle(zr,option,sons[i],p.x,p.y,zlevel);
        			}
        			else
        			{
        				var p=pmap.get(sons[i].id);
        				//var p2=getXY(startX,startY,i,radiuses[3-zlevel]);
        				//var p3=getXY(startX,startY,i,lengths[i]-radiuses[4-zlevel]);
            			//buildLine(zr,p2.x,p2.y,p3.x,p3.y,zlevel);
        				buildLine(zr,startX,startY,p[0],p[1],zlevel);
        			}
        		}
		...
		zr.animate(imageShape.id, "style", false).when(speed, {
			x : startX - r2,
			y : startY - r2,
			width : r2 * 2,
			height : r2 * 2
	...
	var topology = {
		render : function(zr, option) {
			var _self = this;
			option.map=option.data;
			option.pmap = new HashMap();
			if(option.map)
			{
				var cx = zr.getWidth() / 2
				var cy = zr.getHeight() / 2
				cy=cy-65;
				buildCircle(zr,option,option.rootData,cx,cy,4)
			}
...
define('hikcharts/chart/echartsFactory', ['require','hikcharts/chart/topology'],function(require){
	var factory={
		// 创建单指标项百分百环饼图
		createSinglePercentRingPie:function(id, radius, getDataFunction) {
			...
			var radius = radius ? radius : [48, 53];
			var option = {
				animation: true,
				series: [{
					type: 'pie',
			...
			var setValue = function (value, label) {
				option.series[0].data[0].value = value;// (Math.random()*100).toFixed(2)
				// - 0;
				option.series[0].data[1].value = 100 - value;
				option.series[0].data[0].itemStyle.normal.label.formatter = function (a, b, c) {
					return option.series[0].data[0].value + "%"
				}
				option.series[0].data[1].itemStyle.normal.label.formatter = function (a, b, c) {
					return label
				}
				myChart.setOption(option, true);
			}
			...
			// 创建单指标项百分百环饼图
			/*
			 * hikcharts.createSinglePie("mapOverlay",[55,60],function(setValue){
					setValue(60,"测试测试测试测试");
			   });
			 */
			createSinglePie : function(id, radius,getDataFunction) {
					...
					// myChart.setOption(option, true);
					var setValue = function(value, label,color) {
						if(color)
						{
							labelTop.normal.color=color;
							labelTop.normal.label.textStyle.color=color;
						}
			...
			// 创建多指标项百分百环饼图
			/*
			 * hikcharts.createMutliPie("mapOverlay",[55,60],function(setValue){
				  setValue([ 1 ], [ "无数据" ], [ "#ebedf3" ], [ "#9ea1ad" ]);//无数据时
				  setValue([2,5,8],["高：2","中：5","低：8"],["#fd5a5a","#ff9016","#ffc516"]);//有数据时
			   });
			 */
			createMutliPie : function(id, radius, getDataFunction) {
					// 基于准备好的dom，初始化echarts图表
					var myChart = echarts.init(document.getElementById(id));
					var radius = radius ? radius : [ 28, 53 ];
					var option = {
						tooltip : {
							trigger : 'item',
							formatter : "{b}({d}%)"
						...
						series : [ {
							...
							type : 'pie',
							radius : radius,
							//center : [ radius[1]+5, '50%' ],
							center : ['25%', '50%' ],
							data : null
						/*
						 * [ {value:335, name:'排队中：99999',itemStyle:{normal:{ color :
						 * "#ff0000", label : {show : false}, labelLine : {show : false}
						 * }}}, {value:310, name:'执行中：99999',itemStyle:{normal:{ label :
						 * {show : false}, labelLine : {show : false} }}}, {value:234,
						 * name:'已完成：99999',itemStyle:{normal:{ label : {show : false},
						 * labelLine : {show : false} }}} ]
						 */
						} ]
					};
					// myChart.setOption(option, true);
					var setValue = function(values, labels, colors,colors2) {
							...
							option.series[0].data.push({
								...
								itemStyle : {
									normal : {
										color : colors[i],
										label : {
											show : values.length>1?false:true,
											position : 'center',
											textStyle : {
												color:(colors2&&colors2[i])?colors2[i]:colors[i],
												fontSize : '14',
												fontFamily : '微软雅黑',
											    fontWeight : 'bold'
											}
			...
			// 创建折线图
			/*
			 * hikcharts.createLine("mapOverlay",function(setValue) {
						var keys=[ '周一', '周二', '周三', '周四', '周五', '周六', '周日' ];
					    var values=[20,35,40,10,85,90,5];
						setValue("CPU平均利用率",keys,values, "%")
			   },true);
			 */
			createLine : function(id, getDataFunction, hasDataZoom) {
					...
					var option = {
						...
						dataZoom : {
							show : hasDataZoom ? hasDataZoom : false,
							realtime : true,
							start : 0,
							end : 100
						},
					...
					myChart.setOption(option, true);
					var setValue = function(title, data, label, suffix,xformatter) {
						option.legend.data[0].name = title;
						option.series[0].name = title;
						option.xAxis[0].data = label;
						option.series[0].data = data;
						option.tooltip.formatter = '{b}<br/>{a}:{c}'+(suffix ? suffix : "");
						if(xformatter)
						   option.xAxis[0].axisLabel.formatter=xformatter
						option.yAxis[0].axisLabel.formatter = '{value}' + (suffix ? suffix : "");
						myChart.setOption(option, true);
					}
					getDataFunction(setValue);
			},
			// 创建柱状图
			/*
			 * hikcharts.createBar("mapOverlay",function(setValue) {
						var keys=[ '周一', '周二', '周三', '周四', '周五', '周六', '周日' ];
					    var values=[20,35,40,10,85,90,5];
						setValue("CPU平均利用率",keys,values, "%")
			   },true);
			 */
			createBar : function(id, getDataFunction, hasDataZoom) {
					// 基于准备好的dom，初始化echarts图表
					var myChart = echarts.init(document.getElementById(id));
					var option = {
						tooltip : {
						...
						series : [ {
							...
							itemStyle : {
								normal : {
								    ...
								    color:'#1f70a8'
								/*
								 * color:'#bad5fa', lineStyle:{color:'#4c91f1'},
					...
					var setValue = function(title, data, label, suffix,xformatter) {
						...
						if(xformatter)
							 option.xAxis[0].axisLabel.formatter=xformatter
						option.yAxis[0].axisLabel.formatter = '{value}' + (suffix ? suffix : "");
						option.series[0].itemStyle.normal.label.formatter = '{c}' + (suffix ? suffix : "");
						myChart.setOption(option, true);
					}
					getDataFunction(setValue);
			}
 ...
 * hikcharts
 */
define('hikcharts', [ 'require', 'zrender','hikcharts/chart/echartsFactory'],
		function(require, zrender) {
	        var echartsFactory= require('hikcharts/chart/echartsFactory');
			var hikcharts = {
				zrender : null,
				echarts : null,
				target : null,
				init : function(target) {// 图表初始化
					var hc = $.extend({}, this);
			        hc.target=target;
					/*hc.zrender = zrender.init(document.getElementById(target));
					hc.zrender.on('mousewheel', function(a) {
						if (hc.onMouseWheel)
							hc.onMouseWheel(a.event.wheelDelta);
					});*/
					return hc;
				},
				resize : function() {
				...
				dispose : function() {
					...
					//console.log("销毁的ZRENDER：" + this.zrender.id);
					this.zrender=null;
				},
				...
				if (name == "effect") {
				 ...
				 setOption : function(option) {
					if (option.type == "topology") {
						if(!this.zrender)
						{
						  this.zrender = zrender.init(document.getElementById(this.target));
						  this.zrender.on('mousewheel', function(a) {
							if (hc.onMouseWheel)
								hc.onMouseWheel(a.event.wheelDelta);
						  });
						}
						var topology = require('hikcharts/chart/topology');
						topology.hikcharts = this;
						topology.render(this.zrender, option)
					}
						...
						var relation = require('hikcharts/chart/relation');
						relation.hikcharts = this;
						relation.render(this.zrender, option)
					}
					else if (option.type == "earth") {
						if(!this.zrender)
						{
						  this.zrender = zrender.init(document.getElementById(this.target));
						}
						var earth = require('hikcharts/chart/earth');
						earth.hikcharts = this;
						earth.render(this.zrender, option)
					}
					else
					{
						if(!this.echarts)
						  this.echarts = echarts.init(document.getElementById(this.target));
			...
			hikcharts=$.extend({}, hikcharts,echartsFactory);
			return hikcharts;
  });
var hikcharts=require('hikcharts');
_global['hikcharts'] =hikcharts;
